...
/**
 * Feature que diz se um dos termos contêm _somente_ o nome de um país.
 * <p>
 ...
 */
public class FeatureNomePaisApenas extends Feature {
    
    private String regexPaises = "";

    public FeatureNomePaisApenas() {
    ...
    @Override
    public String nome() {
        return "termo_contem_somente_nome_de_pais";
    }
    ...
    public List<Object> gerar(Sentenca s, Termo t1, Termo t2) {
        ...
        for ( Token t : Utilidades.tokensTermo(s, t1) )
            texto += t.getToken() + " ";
        texto = texto.toLowerCase().replace("=", " ").trim();
        
        if ( Pattern.matches(regexPaises, texto) ) {
            valores.add(new Integer(1));
            System.out.println("Pais2Termo1 = " + texto);
        }
        else
        ...
        if ( Pattern.matches(regexPaises, texto) ) {
            valores.add(new Integer(1));
            System.out.println("Pais2Termo2 = " + texto);   
        }
        else
            valores.add(new Integer(0));
